/* 이동 */
function main1(){$('#middle').load('ureader.notice');};

function main2(){$('#middle').load('mreader.notice');};
/* 이동 */
...
function MNoticePageMove(nowPage){
	...
	frm.nowPage.value = nowPage;
	var param = $(frm).serialize();
	$('#middle').load('mreader.notice', param);
};
/*페이지 이동*/
...
function MNoticeView(serial){
	...
	frm.serial.value = serial;
	var param = $(frm).serialize();
	$('#middle').load('mview.notice', param);
};
...
function MNoticeCreateGo(){
	...
	$(frm).findStr = '';
	var param = $(frm).serialize();
	$('#middle').load('mcview.notice', param);
};
